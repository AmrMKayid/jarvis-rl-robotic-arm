...
class DDPG(object):
    ...
    def __init__(self, action_dim, state_dim, action_bound):
        self.memory = np.zeros(
            (MEMORY_CAPACITY, state_dim * 2 + action_dim + 1), dtype=np.float32)
        self.pointer = 0
    ...
    def _build_a(self, s, scope, trainable):
        with tf.variable_scope(scope):
            ...
            a = tf.layers.dense(
                net, self.action_dim, activation=tf.nn.tanh, name='a', trainable=trainable)
    ...
    def _build_c(self, s, a, scope, trainable):
        with tf.variable_scope(scope):
            ...
            w1_s = tf.get_variable(
                'w1_s', [self.state_dim, n_l1], trainable=trainable)
            w1_a = tf.get_variable(
                'w1_a', [self.action_dim, n_l1], trainable=trainable)
